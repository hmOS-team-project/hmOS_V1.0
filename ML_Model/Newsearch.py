...
countt = 0
cnt=0
ssourcepath = 'C://Users/10257/Desktop/PRW-v16.04.20/test_query'
...
def detect(cfg,
           data,
           weights,
           images='C://Users/10257/Desktop/PRW-v16.04.20/testframes',  # input folder
           ...
           save_txt=False,
           save_images=False):
    mylog = open('log.txt', mode='a', encoding='utf-8')
    # Initialize
    ...
    flagg=flagg+1
    print("", file=mylog)
    print(str(flagg), file=mylog)
    reidCfg.DATASETS.ROOT_DIR= (os.path.join(ssourcepath,tempdir))
    #print('tempdir')
   # print(tempdir)
    annotpath = 'C://Users/10257/Desktop/PRW-v16.04.20/testannot'
    files = []
    for file in os.walk(annotpath):
      files.append(file)
    filenames = []
    for dir_t in files:
      if dir_t[2]:
        for filename in dir_t[2]:
          data1 = scio.loadmat(os.path.join(dir_t[0], filename))
          for index in range(data1[list(data1.keys())[3]].shape[0]):
             if int(data1[list(data1.keys())[3]][index][0])==int(tempdir):
               filenames.append(filename)
   # print('filenames is')
  #  print(filenames)
   # print('filenames is',file=mylog)
    #print(filenames,file=mylog)
    ...
    #        print('%gx%g ' % img.shape[2:], end='')  # print image size '288x416'
     ...
     #               print('%g %ss' % (n, classes[int(c)]), end=', ') # 打印个数和类别'5 persons'
                ...
                #    cls_conf = det[i][5]
                    ...
                    if flag == 1:
                        ...
                        #print(crop_img0.shape)
                           cv2.imwrite(
                             'C://Users/10257/Desktop/photo/' + str(xyxy[0]) + str(xyxy[1]) + str(xyxy[2]) + str(
                               xyxy[3]) + '.jpg', crop_img0)
                if save_txt:  # Write to file
                    with open(save_path + '.txt', 'a') as file:
                        file.write(('%g ' * 6 + '\n') % (*xyxy, cls, conf))
            ...
            if gallery_img:
                ...
                if distmat[index] < disminn:
                    disminn=distmat[index]
                    temppath=Path(path).name
    ...
    for filename in filenames:
       if temppath in filename:
            countt=countt+1
            print('countt',file=mylog)
            print(countt,file=mylog)
    print("overlapnum is",file=mylog)
    print(cnt,file=mylog)
    print("overlapnum is ")
    print(cnt)
    print('temppath',file=mylog)
    print(temppath,file=mylog)
    if save_images:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--images', type=str, default='C://Users/10257/Desktop/PRW-v16.04.20/testframes', help='需要进行检测的图片文件夹')
    parser.add_argument('-q', '--query', default=r'C://Users/10257/Desktop/query_box', help='查询图片的读取路径.')
    ...
    for index in range(len(dirlist)):
        ...
        with torch.no_grad():  #数据不需要计算梯度，也不会进行反向传播
             detect(opt.cfg,
                  ...
                  fourcc=opt.fourcc,
                  output=opt.output)
    print('rank1 is '+str(countt/flagg))