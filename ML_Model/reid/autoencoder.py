...
train_dir="C://Users/10257/Desktop/Mobike2017/Mobike2017/DataGenerate/MCPinputX_30000_1000_100.txt"
...
num_hidden_1 = 521 # 1st layer num features
...
with tf.Session() as sess:
    ...
    # Training
      for i in range(0, num_steps):
       ...
       #  print(np.array(result[i]).shape)
       #  print(np.array(result[i]).reshape((-1,1)).shape)
         temp_x=np.array(result[i]).reshape((-1,1)).T
         batch_x = temp_x
            # Run optimization op (backprop) and cost op (to get loss value)
         batch_y,c = sess.run([optimizer, loss], feed_dict={X: batch_x})
        # Display logs per step
         if i % display_step == 0 or i == 1:
              print('epoch %i Step %i: Minibatch Loss: %f' % (e,i, c))

    # Testing
    # Encode and decode images from test set and visualize their reconstruction.
    n = 4
    temp_y=np.array(inputx).reshape((-1,1)).T
    batch_x=temp_y
